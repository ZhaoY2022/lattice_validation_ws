...
Trajectory1dGenerator::Trajectory1dGenerator(const State &lon_init_state, const State &lat_init_state,
                                             ...
                                             const bool lateral_optimization)
...
{
  this->FLAGS_lateral_optimization = lateral_optimization;
}
// 产生轨迹
                                                      ...
                                                      double target_lat_offset)
{
  ...
  GenerateLateralTrajectoryBundle(ptr_lat_trajectory_bundle, driving_mode, current_speed, distance_to_obstacle, target_lat_offset);
}
...
// 产生横向轨迹，即路径规划
void Trajectory1dGenerator::GenerateLateralTrajectoryBundle(Trajectory1DBundle *ptr_lat_trajectory_bundle,
                                                            DrivingMode driving_mode,
                                                            double current_speed,
                                                            double distance_to_obstacle,
                                                            double target_lat_offset) const
{
  if (!FLAGS_lateral_optimization) // 采样规划
  {
    auto end_conditions = end_condition_sampler_.SampleLatEndConditions(driving_mode, current_speed, distance_to_obstacle, target_lat_offset);
  ...
  // QP lateral optimization disabled in standalone test
  // (requires OSQP/qpOASES which are not linked)
}
