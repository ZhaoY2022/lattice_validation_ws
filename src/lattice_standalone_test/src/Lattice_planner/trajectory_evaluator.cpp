...
TrajectoryEvaluator::TrajectoryEvaluator(
    ...
    : path_time_graph_(ptr_path_time_graph), reference_points_(reference_points), init_s_(init_s),
      preferred_lat_sign_(preferred_lat_sign), driving_mode_(driving_mode),
      target_lat_offset_(target_lat_offset)
{
                                     ...
                                     const PtrTrajectory1d &lat_trajectory) const
{
  ...
  // Mode-specific lateral evaluation weights (from configs.yaml)
  double w_lat_offset = Config_.FLAGS_weight_lat_offset;
  double w_lat_comfort = Config_.FLAGS_weight_lat_comfort;

  switch (driving_mode_)
  {
    case DrivingMode::LCC:
      w_lat_offset = Config_.LCC_w_lat_offset;
      w_opposite_side_ = Config_.LCC_w_opposite_side;
      w_same_side_ = Config_.LCC_w_same_side;
      w_lat_comfort = Config_.LCC_w_lat_comfort;
      w_lat_offset_bound_ = Config_.LCC_w_lat_offset_bound;
      break;
    case DrivingMode::ALC:
      w_lat_offset = Config_.ALC_w_lat_offset;
      w_opposite_side_ = Config_.ALC_w_opposite_side;
      w_same_side_ = Config_.ALC_w_same_side;
      w_lat_comfort = Config_.ALC_w_lat_comfort;
      w_lat_offset_bound_ = Config_.ALC_w_lat_offset_bound;
      break;
    case DrivingMode::NUDGE:
      w_lat_offset = Config_.NUDGE_w_lat_offset;
      w_opposite_side_ = Config_.NUDGE_w_opposite_side;
      w_same_side_ = Config_.NUDGE_w_same_side;
      w_lat_comfort = Config_.NUDGE_w_lat_comfort;
      w_lat_offset_bound_ = Config_.NUDGE_w_lat_offset_bound;
      break;
  }

  double total_cost = lon_objective_cost * Config_.FLAGS_weight_lon_objective +
                      ...
                      lat_offset_cost * w_lat_offset +
                      lat_comfort_cost * w_lat_comfort;

  // Tiebreaker: when two otherwise equal trajectories go to opposite sides,
  // prefer the side matching the previous cycle's choice to prevent oscillation.
  if (preferred_lat_sign_ != 0.0 && evaluation_horizon > 0.1)
  {
    double mid_s = evaluation_horizon / 2.0;
    double mid_d = lat_trajectory->Evaluate(0, mid_s);
    if (mid_d * preferred_lat_sign_ < 0.0)
    {
      total_cost += Config_.preferred_lat_sign_tiebreaker;
    }
  }
...
// 1.LatOffsetCost
// Mean squared deviation from target_lat_offset_. Each evaluation point
// contributes ((d - target) / bound)^2 weighted by same/opposite side
// (relative to target, not absolute d=0).
// Overshoot (d beyond target, away from 0) is penalised 100× to bound
// quintic polynomial ringing to ≤0.05m.
    ...
    const std::vector<double> &s_values) const
{
  ...
  for (const auto &s : s_values)
  {
    ...
    double deviation = lat_offset - target_lat_offset_;
    double cost = deviation / w_lat_offset_bound_;
    double sq = cost * cost;
    // Overshoot: d is beyond target (further from 0).  sign(deviation) == sign(target).
    if (target_lat_offset_ != 0.0 && deviation * target_lat_offset_ > 0.0)
    {
      sq *= Config_.ALC_overshoot_penalty;  // heavy penalty — quintic must stay at or inside target
    }
    double start_deviation = lat_offset_start - target_lat_offset_;
    if (deviation * start_deviation < 0.0)
    {
      // trajectory crosses the target — apply opposite-side penalty
      cost_sum += sq * w_opposite_side_;
    }
    else
    {
      cost_sum += sq * w_same_side_;
    }
  }
  return cost_sum / static_cast<double>(s_values.size());
}
