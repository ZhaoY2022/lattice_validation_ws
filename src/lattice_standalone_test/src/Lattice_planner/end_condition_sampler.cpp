...
std::vector<Condition> EndConditionSampler::SampleLatEndConditions(DrivingMode mode, double current_speed, double distance_to_obstacle, double target_lat_offset) const
{
  // Mode-specific lateral sampling arrays
  std::vector<double> end_d_candidates;
  std::vector<double> end_s_candidates;

  switch (mode)
  {
    case DrivingMode::LCC:
      end_d_candidates = Config_.LCC_end_d_candidates;
      {
        double smin = current_speed * Config_.LCC_smin_multiplier;
        for (double delta : Config_.LCC_end_s_deltas)
          end_s_candidates.push_back(smin + delta);
      }
      break;

    case DrivingMode::ALC:
      {
        double tgt = target_lat_offset;
        // end_d = tgt * fraction (from configs.yaml), all on approach side.
        for (double frac : Config_.ALC_end_d_fractions)
          end_d_candidates.push_back(tgt * frac);
        std::sort(end_d_candidates.begin(), end_d_candidates.end());
        end_d_candidates.erase(std::unique(end_d_candidates.begin(), end_d_candidates.end()), end_d_candidates.end());
        // Time-based end_s: t * speed, invariant to speed changes (configs.yaml).
        for (double t : Config_.ALC_end_s_times)
          end_s_candidates.push_back(current_speed * t);
      }
      break;

    case DrivingMode::NUDGE:
      end_d_candidates = Config_.NUDGE_end_d_candidates;
      {
        double safe_smin = std::max(Config_.NUDGE_safe_smin_min,
                                    distance_to_obstacle + Config_.NUDGE_safe_smin_offset);
        double kinematic_smin = current_speed * Config_.NUDGE_kinematic_smin_multiplier;
        double smin_nudge = std::max(safe_smin, kinematic_smin);
        for (double delta : Config_.NUDGE_end_s_deltas)
          end_s_candidates.push_back(smin_nudge + delta);
      }
      break;
  }
