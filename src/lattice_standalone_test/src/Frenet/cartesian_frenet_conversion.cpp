...
/**
 * 
 * rs：车辆轨迹点在Frenet坐标系下的值；
  S条件 (ptr_s_condition)：描述沿参考路径方向的运动状态，包括s, ds/dt, ds2/dt2
 。
D条件 (ptr_d_condition)：描述垂直于参考路径方向的运动状态，包括横向位移d, dd/dt,dd2/dt2
¨
 。
**/
                                                   ...
                                                   std::array<double, 3>* const ptr_d_condition)
{
  ...
  // Normalize to [-pi, pi]: when vehicle heading wraps at ±π but
  // reference heading has not wrapped yet, the raw difference can
  // jump by ±2π and corrupt lateral derivatives (d_dot, d_ddot).
...
double CartesianFrenetConverter::CalculateLateralDerivative(const double rtheta, const double theta, const double l,
                                                            const double rkappa)
{
  return (1 - rkappa * l) * std::tan(common::math::NormalizeAngle(theta - rtheta));
}
                                                                       ...
                                                                       const double rdkappa, const double l)
{
  ...
  const double theta_diff = common::math::NormalizeAngle(theta - rtheta);
