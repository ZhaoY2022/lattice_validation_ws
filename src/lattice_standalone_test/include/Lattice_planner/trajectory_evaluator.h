...
private:
  ...
  double preferred_lat_sign_;  // tiebreaker bias: +1=prefer right, -1=prefer left, 0=no pref
  DrivingMode driving_mode_;   // LCC/ALC/NUDGE for mode-specific cost weights
  double target_lat_offset_;  // desired Frenet d offset (ALC: target lane position)
  mutable double w_opposite_side_;  // mode-specific opposite side offset weight
  mutable double w_same_side_;      // mode-specific same side offset weight
  mutable double w_lat_offset_bound_;  // mode-specific lateral offset bound
};
